...
function _dt_at(s: string; idx:integer=1): Tdatetime; overload;
// By ObServeR
function IntToEvent(ev: integer): string;
function KeywordToInt(keyword: string): integer;
function MessageToHex(data:pointer): string;
function ParseMessageFromRnQ(data:pointer; request:byte = 0): string;
function ParseEvent(data:pointer): string;
function ParseError(data:pointer): string;
function ParseAcknowledge(data:pointer): string;

implementation

function IntlistToStr(list:TintegerDynArray): string;
var
	i: integer;
	delim: string;
begin
...
function ParseGetData(data:pointer; request:byte): string;
begin
	Result := '';
